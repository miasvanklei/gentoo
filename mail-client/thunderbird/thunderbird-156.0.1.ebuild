...
FIREFOX_PATCHSET="firefox-156-patches-03t.tar.xz"
...
IUSE="+clang debug eme-free hardened hwaccel jack +jumbo-build libproxy pgo pulseaudio sndio selinux"
...
src_prepare() {
	...
	sed -i \
		-e 's/\("files":{\)[^}]*/\1/' \
		"${S}"/comm/third_party/rust/audio_thread_priority/.cargo-checksum.json || die
